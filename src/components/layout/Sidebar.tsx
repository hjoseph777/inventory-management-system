...
const Sidebar: React.FC = () => {
  ...
  const menuItems: MenuItem[] = [
    ...
    },
    {
      ...
      icon: 'assessment',
      subMenuItems: [
        { name: 'Inventory Status', path: '/reports/inventory-status', icon: 'inventory' },
        { name: 'Stock Movement', path: '/reports/stock-movement', icon: 'swap_horiz' }
      ]
    },
    {
      name: 'Settings',
                    ...
                    <ul className="sidebar-submenu">
                      {item.subMenuItems.map(subItem => (
                        <li key={subItem.name}>
                          ...
                          >
                            {subItem.icon && <span className="material-icons submenu-icon">{subItem.icon}</span>}
                            {subItem.name}
                          </NavLink>
                        </li>
                      ))}
